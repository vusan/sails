 ...
 */
module.exports.listen = function(bindRouteFn) {
 ...
 */
var fetchRoute = module.exports.fetchRoute = function(url, verb) {
 ...
 * Handle a request
 */
var handleRequest = module.exports.handleRequest = function(target) {
		/**
		 * Handler fn
		 *
...
function handler(req, res, next) {
	...
	function afterwards() {
		...
		if (actionName === 'index') {
			...
			// If no index action exists for this controller
			if (!fn) {
		...
		// viewPath is the path to the view (usually entity/action or entity/'index')
		function afterThat(err, exists, viewPath) {
			...
			// Override next() to trigger the blueprint, but only if it exists
			if (qualifiedForBlueprint(requestedEntity, actionName)) {
				var nextFn = require('./blueprints/controller').definition(requestedEntity)[actionName];
...
function runHandler(req, res, next, fn) {
	var notFoundBlueprint = require('./blueprints/notFound');

	// If no valid route handler can be found or blueprinted, redirect to the default 404 page
	if(!fn || !_.isFunction(fn)) {
		return notFoundBlueprint(req,res);
	}
 ...
 */
var enhanceRequest = module.exports.enhance = require('./request');
